-- S-PROG-13 (expand, ajuste): a função de salvar rascunho por categoria passa a receber as
-- categorias que a tela está editando (`p_categorias`). A tela recalcula descrição e metadata ao
-- salvar, então "categoria sem linha no envio" não pode ser lida como "apagar tudo dela": só as
-- categorias declaradas são substituídas. A versão de 3 parâmetros, criada na migration anterior,
-- ainda não tinha nenhum consumidor e sai.

DROP FUNCTION IF EXISTS public.programacao_salvar_rascunho_categorias(uuid, text, jsonb);
...
-- categoria é feita nela. `p_categorias` diz quais categorias a tela está editando:
--   * categoria fora de `p_categorias`: as linhas do banco ficam intactas e não pode vir linha dela;
--   * categoria em `p_categorias`: precisa "ver"; compara o banco com o que veio da tela. Sem
--     diferença, nada muda.
...
REVOKE ALL ON FUNCTION public.programacao_salvar_rascunho_categorias(uuid, text, jsonb, text[]) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.programacao_salvar_rascunho_categorias(uuid, text, jsonb, text[]) TO authenticated, service_role;
