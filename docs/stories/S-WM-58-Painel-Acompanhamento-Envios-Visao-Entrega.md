...
## Status
InReview — achado 2 do QA Gate CONCERNS (join por FK reversa omitindo disparos pontuais reais) corrigido pelo @dev, ver Change Log 0.5 e Completion Notes abaixo. Achado 1 (enforcement de permissão sem HTTP autenticado de ponta a ponta) segue pendente — não é um achado de código, depende de senha de usuário de teste real ou autorização pra forjar sessão (fora do que o @dev pode resolver sozinho). Recomendado @qa novamente. @devops não acionado, aguardando decisão de Junior.
...
- [x] **Task 6 — Correção do achado de completude do gate CONCERNS** (join por FK reversa omitindo disparos pontuais reais)
  - [x] Causa raiz investigada e confirmada com dado real em produção (não estimada): 2 causas distintas, não 1 — (a) `eventos_pontuais.disparo_id` é ponteiro reverso "só guarda o último", perde disparos antigos quando o mesmo evento dispara mais de uma vez; (b) evento de origem excluído depois via `/configuracoes/programacao` (rota `programacao/excluir`), deixando `disparos.evento_id` órfão (sem FK/constraint entre as tabelas).
  - [x] Migration `20260729090000_swm58_corrige_perda_historico_disparos_pontuais.sql` aplicada em produção via MCP: RPC `listar_disparos_acompanhamento` corrigida — join por FK direta (`ep.id = d.evento_id`, preenchida na criação e nunca sobrescrita) em vez de reversa; motor/inclusão do UNION ALL passam a vir de `disparos.tipo` (confirmado 1:1 confiável — só 2 origens gravam na tabela `disparos`) em vez de depender de match de JOIN; título cai para "(evento removido ou desvinculado)" quando o evento de origem não existe mais, em vez de excluir a linha silenciosamente.
  - [x] `eventos_pontuais.disparo_id`/`ouvidoria_eventos.disparo_id` **não alterados** (schema/comportamento legado pré-existente, fora de escopo, conforme instruído).
  - [x] Teste automatizado (SQL transacional, `BEGIN...ROLLBACK`, nada persiste): `supabase/tests/swm58_listar_disparos_acompanhamento_historico.sql` — cobre especificamente evento com 2 disparos (caso que sumia) + evento excluído depois. Mutation check feito ao vivo: rodado o mesmo cenário ANTES da correção (RPC antiga, join reverso) → só 1 dos 2 disparos sintéticos encontrado (bug reproduzido); DEPOIS de aplicar a correção → os 2 + o caso de evento excluído aparecem corretos. `ROLLBACK` confirmado sem deixar traço (`count(*) = 0` pós-teste).
  - [x] Confirmado ao vivo com dado real de produção: `listar_disparos_acompanhamento(p_motor:='pontual')` agora retorna os **10** disparos pontuais reais que existem em `disparos` (antes: 5) — os 4 que a QA apontou como faltantes aparecem (1 recuperado pelo join direto — evento "Corrida da Juventude" com 2 disparos; 3 com título genérico — eventos excluídos depois). Números de `total_enviados`/`total_entregues`/`total_falhou` batem exatamente com `logs_disparo` para os disparos que já apareciam antes (sem regressão). `divulgacao` (28) e `ouvidoria` (0) inalterados.
  - [x] **Nota de transparência, não uma correção pendente:** o total real passou a ser **10**, não os "9" citados no achado da QA — a diferença é 1 disparo (`91ed62f2...`, `evento_id` sentinela `00000000-...-057`) que a QA classificou como "sintético de validação da própria S-WM-57, esperado ficar de fora". Decidido conscientemente **não** excluí-lo via um `WHERE disparo_id <> '91ed62f2...'` hardcoded na RPC: esse registro tem `total_destinatarios=4` e 4 linhas reais em `logs_disparo` (mensagens de fato enviadas) — é um disparo real, só não veio do fluxo normal de evento. Hardcodear a exclusão de 1 UUID específico numa função de produção seria gambiarra, não correção de causa raiz. Fica visível no painel com título genérico; se Junior quiser esse registro específico fora da visão de entrega, é uma decisão de dado (apagar a linha ou marcá-la) e não uma correção de query.
...
- `supabase/migrations/20260729090000_swm58_corrige_perda_historico_disparos_pontuais.sql` (novo — aplicado em produção: corrige RPC `listar_disparos_acompanhamento` para não perder histórico de disparos pontuais)
- `supabase/tests/swm58_listar_disparos_acompanhamento_historico.sql` (novo — teste de regressão transacional para o achado de completude do gate CONCERNS)
...
| 2026-07-29 | 0.5 | Corrigido o achado de completude do gate CONCERNS (join por FK reversa omitindo disparos pontuais reais). Causa raiz investigada com dado real, não estimada: 2 causas distintas — (a) `eventos_pontuais.disparo_id` guarda só o último disparo do evento (achado original da QA); (b) evento de origem excluído depois via `/configuracoes/programacao`, achado novo encontrado durante a investigação, deixando `disparos.evento_id` órfão (sem FK/constraint entre as tabelas). Migration `20260729090000` aplicada em produção: RPC passa a usar join direto (`disparos.evento_id → eventos_pontuais.id`, preenchido na criação e nunca sobrescrito) em vez de reverso, motor/inclusão vêm de `disparos.tipo` (confirmado 1:1 confiável) em vez de depender de match de JOIN, título cai para fallback genérico quando o evento não existe mais em vez de excluir a linha. `eventos_pontuais.disparo_id`/`ouvidoria_eventos.disparo_id` não alterados (fora de escopo, conforme instruído). Teste de regressão SQL transacional (`supabase/tests/swm58_listar_disparos_acompanhamento_historico.sql`) criado e validado com mutation check ao vivo (bug reproduzido antes da correção, resolvido depois, rollback sem deixar traço). Confirmado com dado real: RPC agora retorna 10 disparos pontuais (antes: 5) — não 9 como estimado pela QA, porque 1 disparo real de teste da própria S-WM-57 (com `logs_disparo` reais) também passou a aparecer, por decisão consciente de não hardcodear exclusão de 1 UUID específico na função (ver Task 6 para detalhe). Números dos disparos que já apareciam antes conferem exatamente com `logs_disparo`, sem regressão. Achado 1 do gate (enforcement sem HTTP autenticado de ponta a ponta) não é um achado de código — segue pendente, depende de credencial de teste real ou autorização explícita, fora do que o @dev pode resolver sozinho. Status mantido InReview, recomendado @qa novamente. @devops não acionado. | @dev Dex |
