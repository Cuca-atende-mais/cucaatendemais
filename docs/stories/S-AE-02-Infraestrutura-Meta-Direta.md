...
- [ ] **NOVO (2026-08-20, corrigido pelo @po):** ajustar `developer/meta-numeros/page.tsx` — incluir `"academia_enem"` em `AGENTES_META` **e** em `CANAL_TIPOS_META` (minúsculo, snake_case — **não** `"Academia Enem"` capitalizado), mais a entrada correspondente em `CANAL_BADGE_COLORS`. O valor exato de `canal_tipo` importa: já é lido como `"academia_enem"` (minúsculo) pelo painel de atendimento (`CANAL_ACADEMIA_ENEM` em `academia-enem/mensagens/page.tsx`, S-AE-03) e pela migration seed já preparada — um valor capitalizado quebraria o filtro `filterCanalTipo` do painel. Bloqueante antes do cadastro real do número via tela (substitui a migration manual como mecanismo).
