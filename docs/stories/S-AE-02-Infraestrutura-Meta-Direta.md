...
| `WORKER_SCOPE` | `academia_enem` | **OBRIGATÓRIA.** Sem essa variável, o serviço roda o mesmo código do `cuca-worker` e tentaria reivindicar/enviar disparos do Institucional/Empregabilidade/Ouvidoria/Divulgação com o token Meta errado da Academia Enem — achado levantado e corrigido em 2026-08-21 (ver Change Log), gate em `worker/main.py`. |
...
| 2026-08-21 | @dev (Dex) | **Achado bloqueante, levantado ao iniciar a S-AE-09:** `campanhas_loop()` (`worker/campanhas_engine.py`) — que processa disparos do Institucional/Empregabilidade/Ouvidoria/Divulgação — roda sem filtro de escopo, e o serviço `cuca-academia-enem` desta story usa a MESMA imagem/`Dockerfile` (decisão já registrada acima na seção de infraestrutura). Sem correção, ao implantar o serviço novo, ele reivindicaria e tentaria enviar disparos de outros módulos com o token Meta da Academia Enem. Corrigido em `worker/main.py` com um gate `WORKER_SCOPE` (ver Change Log/File List da S-AE-09, onde a correção foi implementada). **Variável `WORKER_SCOPE=academia_enem` adicionada como OBRIGATÓRIA** na tabela de ambiente desta story (seção "Variáveis de ambiente do serviço `cuca-academia-enem`") — sem ela, o serviço novo repete o comportamento do `cuca-worker` e o problema volta. Nenhuma Task de infraestrutura desta story deve ser executada sem incluir essa variável. |
