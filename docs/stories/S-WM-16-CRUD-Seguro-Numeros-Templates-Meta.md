...
| 2026-07-04 | @dev (Dex) | **Bug crítico encontrado durante smoke test da S-WM-18 e corrigido aqui (origem do bug é esta story):** o lookup relacional de templates introduzido nesta story usa `.eq("automacoes", [tag])` — uma lista Python/JS — em vez da sintaxe de array literal do Postgres. O cliente `postgrest-py` serializa a lista como `str(list)` (`"['Institucional']"`), que o Postgrest rejeita com 400 (`malformed array literal`); no worker isso faz `.maybe_single()` levantar exceção não tratada, travando a fila/item para sempre em `em_andamento` (nunca reprocessado, pois a query só relê `status='pendente'`). Reproduzido ao vivo 2x na fila de smoke test da S-WM-18 (`disparos_divulgacao`) e confirmado o erro exato via SQL puro (`automacoes = 'Institucional'::text[]` → `malformed array literal`). Mesma falha de serialização confirmada por leitura de código em 3 outros pontos com o padrão idêntico: `worker/campanhas_engine.py::processar_item_disparo` (eventos_pontuais/ouvidoria_eventos, não testado ao vivo — canal Divulgação sem número cadastrado impede reprodução real, mas o código é byte-a-byte igual ao ponto já comprovado), `cuca-portal/.../divulgacao/disparar/route.ts` e `cuca-portal/.../empregabilidade/vagas/feedback-submit/route.ts` (cliente JS serializa array via `toString()`, gerando `eq.Institucional` sem chaves — mesmo erro Postgres, reproduzido por SQL). **Correção aplicada nos 4 pontos:** manter `.eq()` (não trocar por `.contains()`, que reabriria colisão com templates multi-tag — ver comentário original do código), mas passando a string de array literal do Postgres (`'{"Institucional"}'`) em vez de uma lista. Adicionado tratamento de erro gracioso em `processar_item_disparo`/`processar_disparos_divulgacao`: qualquer exceção não prevista agora marca o item/fila como `pausada`/`erro` e loga com stack trace completo, em vez de travar silenciosamente em `em_andamento`. `pytest worker/tests/` 74 passed/3 skipped, sem regressão. Autorizado por Junior como correção de escopo ampliado (não é alteração da S-WM-18, é reparo de bug real desta story), documentado também na S-WM-18 por ter sido descoberto lá. |
| 2026-07-04 | @qa (Quinn) | Quality gate independente da correção acima: confirmei por grep os 4 pontos corrigidos, testei a query corrigida via SQL direto (`'{"Institucional"}'::text[]` casa com o template real), confirmei que `automacao_tag`/o literal usado na concatenação de string sempre vem de valores hardcoded no próprio código (sem risco de injeção), e rodei `pytest worker/tests/` de forma independente (74 passed/3 skipped, sem regressão). PASS. Veredito completo e achados adicionais (RLS de `disparos_divulgacao`, AC9 pendente de redeploy) documentados no QA Results da S-WM-18. |
| 2026-07-04 | @dev (Dex) | **2º bug encontrado pós-deploy do fix anterior, mais perto da causa raiz definitiva:** query corrigida e template encontrado, mas o envio real à Graph API retornou HTTP 400 `(#100) Invalid parameter... Parameter name is missing or empty`. Junior confirmou direto no Business Manager: `institucional_programacao_mensal_v1` foi aprovado na Meta com **variáveis NOMEADAS** (`{{nome}}`, `{{mes}}`), não posicionais numéricas como nosso `corpo_texto`/`variaveis` local sugeriam (essa era uma suposição da S-WM-16 que se provou incompleta — a conversão para exibição `{{1}}/{{2}}` não implica que o template aprovado na Meta seja NUMBERED). A Graph API exige `parameter_name` em cada parâmetro do body quando o template é NAMED. Corrigido com função genérica `_montar_parametros_named()` em `campanhas_engine.py`, que deriva o `parameter_name` de `meta_templates.variaveis[].descricao` (já cadastrado, sem hardcode) — aplicada por ora só no ponto confirmado (Divulgação/`institucional_programacao_mensal_v1`). Query do template ampliada para também selecionar `variaveis`. `pytest worker/tests/` 74/3 sem regressão. **Achado de escopo mais amplo:** o mesmo padrão de envio (sem `parameter_name`) existe em outros 2 pontos ativos — `_processar_item_disparo_interno` (eventos_pontuais/ouvidoria, hoje dormente, 0 linhas) e `_notificar_transbordo` em `meta_adapter_inbound.py` (handover para humano, **ativo em produção** — se `institucional_transbordo_v1` também for NAMED, notificações de transbordo podem estar falhando silenciosamente, só logado como warning). Correção desses 2 pontos fica pendente de confirmação do Junior no Business Manager antes de aplicar (evitar trocar um erro por outro). Também identificado, sem corrigir agora (fora de escopo desta correção pontual): possível desalinhamento semântico em `institucional_programacao_pontual_v1` (posição 1 espera "nome" mas o código envia "titulo" do evento) — dormente, sem urgência. Recomendado a Junior avaliar com `@po` adicionar coluna `parameter_format` (NAMED/NUMBERED) em `meta_templates` como item separado, para não depender de suposição nesse ponto de novo. |
| 2026-07-04 | @dev (Dex) | Junior confirmou no Business Manager: `institucional_programacao_pontual_v1` (nome, titulo_evento, descricao_evento, data_evento, horario_evento, local_evento) e `institucional_transbordo_v1`/`empregabilidade_transbordo_v1` (colaborador, lead, canal) também são **NAMED**. Aplicado `_montar_parametros_named()` nos 2 pontos restantes: `_processar_item_disparo_interno` (`campanhas_engine.py`, branches eventos_pontuais e ouvidoria_eventos — ambas usam a mesma função, dormentes hoje mas corrigidas para quando tiverem tráfego) e `_notificar_transbordo` (`meta_adapter_inbound.py`, handover ativo em produção — importa `_montar_parametros_named` de `campanhas_engine`, junto com `_enviar_template_meta` já importado antes). Queries dos 2 pontos ampliadas para selecionar `variaveis`. `pytest worker/tests/` 74/3 sem regressão. **Todos os 3 pontos de disparo de template Meta do worker agora usam `parameter_name` derivado do banco, sem hardcode.** Mantido sem correção (fora de escopo, já registrado acima): possível desalinhamento semântico de ordem de valores em `institucional_programacao_pontual_v1` (dormente). Aguardando Junior chamar `@devops` para push + redeploy e criar a 4ª fila de smoke test — nada foi deployado/testado ao vivo ainda nesta rodada. |
| 2026-07-04 | @qa (Quinn) | Quality gate independente dos 3 pontos com `_montar_parametros_named()`: confirmei por grep que os 3 call sites e as 3 queries (com `variaveis` no select) estão presentes; busquei `variaveis` reais dos 4 templates envolvidos no cuca-dev e testei a função isoladamente com esses dados exatos — mapeamento de `parameter_name` correto para Divulgação (nome, mes), pontual (6 vars) e transbordo (colaborador, lead, canal). `pytest worker/tests/` reconfirmado 74/3. **Achado (CONCERNS, não bloqueia):** os testes existentes de `_notificar_transbordo` (`test_meta_adapter_inbound.py`) retornam cedo em todos os 3 casos (template não encontrado ou sem contatos) — nenhum exercita o caminho feliz que chama `_montar_parametros_named`. Não existe `test_campanhas_engine.py` — `processar_disparos_divulgacao`/`processar_item_disparo` seguem sem cobertura automatizada (pré-existente, não introduzido por este fix). `httpx` não está instalado neste sandbox, então `campanhas_engine.py` nunca chega a ser importado durante os testes aqui — validado só por `py_compile` + teste manual da função pura + conferência dos dados reais do banco. Recomendo ao @dev adicionar teste unitário de `_montar_parametros_named()` (função pura, fácil de testar, sem mock de Supabase) como follow-up — não bloqueante, aprovado para push. Verdict: **CONCERNS**. |
| 2026-07-04 | @qa (Quinn) | `*close-story`: revisão final antes de Done. Escopo original (Tasks 1-3, 5) já validado em turnos anteriores com reconciliação de ACs pelo @po; Task 4 (validação cruzada com API real da Meta) permanece intencionalmente não iniciada — gate com @po, já documentado como decisão de escopo, não é lacuna silenciosa. Os 2 ciclos de bug real descobertos durante o smoke test da S-WM-18 (sintaxe de array Postgres; `parameter_name` para templates NAMED) foram corrigidos, passaram por QA gate independente (CONCERNS ambos, sem CRITICAL/HIGH) e estão em `origin/develop`. Confirmado com dado real do cuca-dev: 4ª fila de smoke test da S-WM-18 (`39452999-...`) concluiu com `total_enviados=1`, prova de que a correção funciona em produção real (staging), não só em teoria. **Débitos registrados para acompanhar, não bloqueiam o fechamento:** (1) teste unitário de `_montar_parametros_named()` ausente; (2) coluna `parameter_format` em `meta_templates` — decisão de escopo pendente com @po; (3) possível desalinhamento semântico dormente em `institucional_programacao_pontual_v1`. Status → **Done**. |
