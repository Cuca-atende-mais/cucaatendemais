...
- [ ] **Task 5 — Testes automatizados e regressão** (AC: 1–12)
  ...
  - [x] Adicionar cobertura possível para o guard visual e preview, seguindo a infraestrutura existente; se não houver harness frontend, documentar e executar smoke manual. Sem harness frontend (só `logic.ts` tem teste automatizado). Smoke manual executado por Junior em 2026-07-04 direto no staging: confirmados os 4 pontos — sem chip/QR/UAZAPI na tela, número Meta Institucional exibido corretamente, preview do template real no modal, botão habilitado.
...
### `*close-story` — 2026-07-04

AC9 confirmado com dado real: 4ª fila de smoke test (`39452999-...`) concluiu `total_enviados=1`, `total_erros=0`. Cadeia completa de bugs do fluxo outbound (array Postgres → `parameter_name` NAMED) corrigida e validada em staging real, não só em teoria.

**Não estou marcando `Done` ainda — um item genuíno continua em aberto, não é só limitação de ambiente:** Task 5 exige "smoke manual" do guard visual/preview quando não há harness frontend (não há — confirmei que só existe teste de `logic.ts`, nada que renderize o componente React de verdade). **Esse smoke manual nunca foi executado nesta sessão.** Não tenho como fazer isso eu mesmo: não há `.env.local` no `cuca-portal` nem credenciais Supabase funcionais neste ambiente para subir o dev server contra dados reais — verifiquei antes de reportar isso, não é suposição.

Isso é diferente do CodeRabbit/build (limitações de ambiente já aceitas ao longo de toda a sessão) — é a única verificação end-to-end da UI que ninguém fez ainda: confirmar ao vivo, logado no staging, que o botão habilita/desabilita corretamente, o modal abre com o preview certo, e não sobrou nada de chip/QR na tela. Como é rápido (2-3 minutos) e você já tem acesso ao staging, recomendo você mesmo fazer essa checagem visual antes de eu marcar `Done` — ou, se preferir aceitar o risco e fechar mesmo assim, me diga explicitamente e eu registro como exceção aceita (não decido isso sozinho).

**Atualização — 2026-07-04:** Junior confirmou visualmente no staging os 4 pontos do smoke manual (sem chip/QR/UAZAPI; número Meta Institucional exibido corretamente; preview do template real no modal; botão habilitado). Gap fechado com evidência real, não é mais exceção aceita — é verificação concluída. Task 5 e Task 6 completas. AC9 já confirmado (4ª fila de smoke test). **Nenhum item bloqueante restante.** Status → **Done**.

**Débitos registrados para acompanhamento futuro (não bloqueiam o fechamento):**
- Achado de segurança: RLS de `disparos_divulgacao` permite INSERT/UPDATE irrestrito para qualquer autenticado — recomendado ticket separado (`has_permission()`).
- Teste unitário de `_montar_parametros_named()` ausente (ver S-WM-16).
- Coluna `parameter_format` (NAMED/NUMBERED) em `meta_templates` — decisão de escopo pendente com `@po`.
- Possível desalinhamento semântico dormente em `institucional_programacao_pontual_v1` (posição 1 "nome" recebendo "titulo" do evento).
- Pre-Commit/Pre-PR/Pre-Deployment CodeRabbit não executados (indisponível neste ambiente ao longo de toda a sessão).
...
| 2026-07-04 | @dev (Dex) | 2º bug pós-redeploy: query e template resolvem, mas Graph API rejeitou com HTTP 400 `(#100) Parameter name is missing or empty`. Junior confirmou no Business Manager que `institucional_programacao_mensal_v1` é NAMED (`{{nome}}`, `{{mes}}`), não NUMBERED como a suposição original da S-WM-16. Corrigido com `_montar_parametros_named()` (deriva de `meta_templates.variaveis`, sem hardcode) — detalhe completo no Change Log da S-WM-16, incluindo achado de escopo mais amplo (mesmo padrão em `_processar_item_disparo_interno` e `_notificar_transbordo`, este último ativo em produção). |
| 2026-07-04 | @qa (Quinn) | 2º quality gate: validação independente dos 3 pontos corrigidos (grep, dados reais de `variaveis` no cuca-dev, pytest 74/3 reconfirmado). Veredito **CONCERNS**: aprovado para push — achado de cobertura de teste ausente no caminho feliz (não bloqueante, ver Change Log da S-WM-16). |
| 2026-07-04 | @dev (Dex) | Pós-redeploy do fix de `parameter_name`: 4ª fila de smoke test (`39452999-5cae-4596-8d5f-4ecd364778ae`) concluiu com **`total_enviados=1`, `total_erros=0`** — primeiro envio real bem-sucedido de ponta a ponta. AC9 confirmado com evidência real, não apenas ausência de erro. |
| 2026-07-04 | @qa (Quinn) | `*close-story`: revisão final. AC9 confirmado com dado real. Identificado item genuíno pendente (não limitação de ambiente): smoke manual visual da UI (Task 5) nunca foi executado nesta sessão — sem credenciais locais para rodar o portal, não pude fazer eu mesmo. Recomendado a Junior confirmar visualmente no staging antes do fechamento. |
| 2026-07-04 | Junior | Confirmação visual direta no staging: sem chip/QR/UAZAPI na tela, número Meta Institucional exibido corretamente, preview do template real no modal, botão habilitado conforme esperado. Os 4 pontos do smoke manual da Task 5 verificados com evidência real. |
| 2026-07-04 | @qa (Quinn) | `*close-story` finalizado: Task 5 e Task 6 completas, AC9 confirmado, nenhum item bloqueante restante. Débitos técnicos (RLS de `disparos_divulgacao`, teste unitário de `_montar_parametros_named()`, coluna `parameter_format`, desalinhamento dormente em `institucional_programacao_pontual_v1`, CodeRabbit não executado) registrados para acompanhamento futuro, não bloqueiam. **Status → Done.** |
