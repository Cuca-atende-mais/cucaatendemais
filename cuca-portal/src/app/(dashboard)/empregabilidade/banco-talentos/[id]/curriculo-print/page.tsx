...
export default function CurriculoPrintPage() {
    ...
    useEffect(() => {
        supabase
            .from("talent_bank")
            .select("curriculo_estruturado, nome")
            .eq("id", talentId)
            .single()
            .then(({ data, error }) => {
                if (error || !data) { setErro("Candidato não encontrado."); setLoading(false); return }
                const cur = data.curriculo_estruturado
                if (!cur || Object.keys(cur).length === 0) {
                    setErro("Este candidato ainda não tem um currículo estruturado. Use o Construtor de Currículo para criar um.")
                    setLoading(false)
                    return
                }
                setCurriculo({ nome: data.nome, ...cur } as CurriculoData)
                setLoading(false)
            })
    }, [talentId])
        ...
        <>
            ...
            <div className="print:mt-0 mt-16 min-h-screen bg-white text-black">
                ...
                >
                    ...
                    <div style={{ textAlign: "center", marginBottom: "20px" }}>
                        <h1 style={{ fontSize: "26px", fontWeight: 700, letterSpacing: "-0.5px", margin: "0 0 6px" }}>{curriculo.nome}</h1>
                        {contatos && (
                    ...
                    {curriculo.objetivo && (
                        <section style={{ marginBottom: "20px" }}>
                            ...
                            <p style={{ margin: "6px 0 0", color: "#333" }}>{curriculo.objetivo}</p>
                        </section>
                    )}
            ...
            <style>{`
                @media print {
                    ...
                    /* Remove margens do Next.js layout */
